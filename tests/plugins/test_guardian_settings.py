"""Tests for the user-configurable Guardian detection policy.

Covers the whole configurable-policy surface that ships together:

* :class:`config.GuardianConfig` per-category policy (``category_setting`` /
  ``category_allows``) and how ``load_guardian_config`` parses the new
...
* :mod:`settings` write/read round-trip persisting to the tmpdir-isolated
  ``HERMES_HOME/config.yaml`` so a fresh ``load_guardian_config`` sees it.
...
# ---------------------------------------------------------------------------
# 1. GuardianConfig.category_setting / category_allows
# ---------------------------------------------------------------------------
...
def test_category_min_severity_critical_rejects_below():
    cfg = config_mod.GuardianConfig(categories={"dependency": {"min_severity": "critical"}})
...
def test_category_disabled_rejects_everything():
    cfg = config_mod.GuardianConfig(categories={"fileaccess": {"enabled": False}})
...
def test_category_garbage_min_severity_falls_back_to_info():
    cfg = config_mod.GuardianConfig(categories={"injection": {"min_severity": "banana"}})
...
def test_category_setting_ignores_non_mapping_entry():
    ...
    cfg = config_mod.GuardianConfig(categories={"skill": "enabled"})
    assert cfg.category_setting("skill") == {"enabled": True, "min_severity": "info"}


# ---------------------------------------------------------------------------
# 2. load_guardian_config parses detection.<cat> + protected_paths
# ---------------------------------------------------------------------------
...
def test_flag_worthy_disabled_category_drops_public_confirmed():
    ...
    cfg = config_mod.GuardianConfig(categories={"dependency": {"enabled": False}})
...
def test_on_pre_tool_call_blocks_custom_protected_path(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda: config_mod.GuardianConfig(
...
def test_report_and_audit_never_shares_custom_finding(monkeypatch):
    ...
    cfg = config_mod.GuardianConfig(report=True)
    custom = detection.Finding(
        identifier="fileaccess:read_file:user-protected",
        category="fileaccess",
        severity="critical",
