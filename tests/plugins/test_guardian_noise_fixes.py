...
``HERMES_HOME``/``GUARDIAN_HOME`` are per-test tmpdirs (root conftest).
"""
...
def test_record_activity_logs_shell_installs_and_reads():
    import json
    from plugins.guardian import constants
    ...
    deps = [json.loads(ln) for ln in (constants.guardian_home() / "dependencies.jsonl").read_text().splitlines() if ln.strip()]
