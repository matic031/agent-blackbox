"""Tests for the Guardian three-tier trust model (public / community / heuristic).
...
def _block_cfg():
    return config_mod.GuardianConfig(mode="block", block_severity="critical")
...
def test_block_mode_community_critical_match_never_blocks(monkeypatch):
    ...
    monkeypatch.setattr(config_mod, "load_guardian_config", _block_cfg)
...
def test_flag_worthy_drops_heuristic_below_report_min_severity():
    cfg = config_mod.GuardianConfig()  # report_min_severity defaults to "high"
