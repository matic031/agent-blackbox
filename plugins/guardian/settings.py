"""Read/write the user-tunable Guardian detection policy.

The dashboard settings page (gear icon) is the primary consumer. Settings live
under ``plugins.entries.guardian.*`` in the hermes ``config.yaml`` — the same
place :mod:`config` reads from — so a change here is picked up by every hook on
the next :func:`config.load_guardian_config`.
...
from .config import DETECTION_CATEGORIES, load_guardian_config

logger = logging.getLogger(__name__)

_GEAR_KEYS = ("plugins", "entries", "guardian")
...
def _validate(payload: Dict[str, Any]) -> Tuple[Dict[str, Any], List[str]]:
    """Coerce a settings *payload* into a validated ``guardian`` config subtree.
...
def write_settings(payload: Dict[str, Any]) -> Dict[str, Any]:
    """Validate *payload* and persist it under ``plugins.entries.guardian``.
...
def _persist(updates: Dict[str, Any]) -> bool:
    """Merge *updates* into the guardian config entry. Prefer hermes' writer."""
    try:
        ...
        logger.debug("guardian.settings: hermes config write failed (%s); trying YAML", exc)
    ...
    try:
        ...
        logger.debug("guardian.settings: YAML config write failed: %s", exc)
        return False
