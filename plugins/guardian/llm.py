...
Guardian's core detection is deterministic (regex + graph). This module adds a
*second opinion*: when the user has configured an LLM (``llm.enabled`` with a
provider/model/key), Guardian can ask it whether a message looks like a
...
def review_injection(text: str, cfg: Any) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        logger.debug("guardian.llm: review call failed: %s", exc)
        return None
...
def _post(url: str, headers: Dict[str, str], body: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        logger.debug("guardian.llm: HTTP %s from %s: %s", exc.code, url, exc.read(512).decode("utf-8", "replace"))
        return None
    except (urllib.error.URLError, OSError, TimeoutError) as exc:
        logger.debug("guardian.llm: transport error to %s: %s", url, exc)
        return None
    try:
